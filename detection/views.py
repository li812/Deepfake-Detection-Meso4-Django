    ...
    if os.path.isfile(input_path):
        if input_path.lower().endswith((".png", ".jpg", ".jpeg")):
            ...
            if frame is not None:
                ...
                if prediction <= 0.8:
                    cv2.imwrite(
                        f"{output_folder}/frame_{os.path.basename(input_path)}.jpg",
                        frame,
                    )
...
def index(request):
    ...
    if request.method == "POST" and request.FILES["file"]:
        ...
        if predict_fake_frames(full_file_path, output_folder, meso):
            ...
            detected_frames = [
                os.path.join(settings.MEDIA_URL, "Detected_Fake_Frames", f)
                for f in os.listdir(output_folder)
            ]
            # Delete the uploaded file after detection
            os.remove(full_file_path)  # Add this line to delete the uploaded file
            return render(
                request, "detection/index.html", {"detected_frames": detected_frames}
            )
        else:
            ...
            return render(
                ...
                {"message": "No deepfaked frames detected."},
            )
